...
// 缓存core请求的信号
 ...
 // Interconnect With Core & AXI
  ...
  data_read2core   := MuxLookup(req_offset(3), 0.U, Array(
...
switch (state) {
  ...
  data_ready2core   :=RegNext(true.B) //读需要延后一拍等cache_data_out
...
axi.data_addr_r  := data_addr_r2axi 
axi.data_addr_w  := data_addr_w2axi 
